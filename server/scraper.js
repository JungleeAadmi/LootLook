...
async function scrapeProduct(url) {
    ...
    try {
        ...
        // 2. NAVIGATION
        try {
            // Extended timeout for slow redirects (Savana/Flipkart share links)
        ...
        const data = await page.evaluate(() => {
            ...
            // --- STRATEGY 1: META TAGS (Often best for Savana/Tata) ---
            ...
            // --- STRATEGY 3: VISUAL SELECTORS (Site Specifics) ---
            if (!price || price === 0) {
                const selectors = [
                    // Quartz / Silverline / Shopify General
                ...
                for (let sel of selectors) {
                    ...
                    if (el && el.innerText.match(/[0-9]/)) {
                        ...
                        // Simple check to avoid descriptions
                        if(txt.length < 30) {
                            price = txt;
                            if (!currency) {
            ...
            if (!image) {
                const imgSelectors = [
                    ...
                    '.ProductDetailsMainCard__galleryImage img', // Tata
                    'img._396cs4', '#landingImage'
                ];
        ...
        // --- CURRENCY FORCING ---
        // Force INR for specific domains even if they are .com
        let finalCurrency = data.currency;
        const currentUrl = page.url().toLowerCase();
        
        const indianSites = [
            '.in', 
            'flipkart', 'meesho', 'tatacliq', 'myntra', 'ajio', 
            'quartzcomponents', 'robocraze', 'savana', 'silverline'
        ];
