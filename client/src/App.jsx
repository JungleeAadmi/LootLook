...
const Icons = {
  ...
  Snip: () => <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
            ...
            {filteredItems.map(item => (
                <article key={item.id} className={`item-card trend-${getTrend(item.current_price, item.previous_price)}`}>
                    <div className="card-snip" onClick={(e) => openImage(e, item)}>
                        ...
                        <span className="snip-tag">Snip</span>
                    </div>
                    ...
                    <div className="card-actions">
                        ...
                        <a href={item.url} target="_blank" rel="noreferrer" className="action-btn visit" title="Visit Store"><Icons.Visit /></a>
                        <button onClick={() => handleDelete(item.id)} className="action-btn remove" title="Remove Item"><Icons.Remove /></button>
                    </div>
                </article>
            ))}
