...
const Icons = {
  ...
  Undo: () => <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
};
...
function App() {
  ...
  const handleDelete = async (id) => { 
      // Immediate soft delete request
      await fetch(`${API_URL}/items/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } }); 
      setDeletedItem(id); // Store ID for potential restore
      setToastVisible(true);
      // Hide toast after 5 seconds
      setTimeout(() => { setToastVisible(false); setDeletedItem(null); }, 5000);
  };

  const handleRestore = async () => {
      if (!deletedItem) return;
      await fetch(`${API_URL}/items/${deletedItem}/restore`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } });
    ...
    <div className={`app-wrapper ${theme}`}>
      ...
      <nav className="navbar">
        <div className="nav-content">
            ...
            <div className="nav-actions"><button className={`nav-btn ${globalSync ? 'spin' : ''}`} onClick={fetchItems} title="Sync"><Icons.Sync /></button><button className="nav-btn" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <Icons.Close /> : <Icons.Menu />}</button>{menuOpen && (<div className="menu-dropdown"><div className="menu-item" onClick={handleCheckAll}>⚡ Check All</div><div className="menu-item" onClick={handleExport}>📥 Export CSV</div><div className="menu-item" onClick={toggleTheme}>{theme === 'dark' ? '☀️ Light Mode' : '🌙 Dark Mode'}</div><div className="menu-item danger" style={{color: 'var(--danger)'}} onClick={logout}>Logout</div></div>)}</div>
        </div>
      </nav>
      <main className="main-content">
        {/* ... [Controls] ... */}
        <section className="controls-panel"><form onSubmit={handleAdd} className="add-bar"><input type="url" placeholder="Paste product link here..." value={url} onChange={(e) => setUrl(e.target.value)} required className="main-input" /><div className="add-actions"><select value={retention} onChange={(e) => setRetention(e.target.value)} className="main-select"><option value="30">30 Days</option><option value="90">90 Days</option><option value="365">1 Year</option></select><button type="submit" disabled={loading} className="primary-btn">{loading ? 'Adding...' : 'Track'}</button></div></form><div className="filter-bar"><label>Filter:</label><select onChange={(e) => setFilterDomain(e.target.value)} value={filterDomain} className="filter-select">{uniqueDomains.map(d => <option key={d} value={d}>{d}</option>)}</select></div></section>
                ...
                <article key={item.id} className={`item-card trend-${getTrend(item.current_price, item.previous_price)}`}>
                    ...
                    <div className="card-body" onClick={() => openHistory(item)}>
                        ...
                        <div className="meta-row">
                            {renderPriceBox(item)}
                            <div className="timestamps"><span className="date-added">Added: {formatDate(item.date_added)}</span><span className="separator">|</span><span className="date-checked">Checked: {formatDate(item.last_checked)} {formatTime(item.last_checked)}</span></div>
                        </div>
                    ...
                    <div className="shared-section">
                        ...
                        <div className="user-list existing-shares">
                             {getSharedWithList(shareItem).map(u => (
                                <div key={u.id} className="user-item"><span>{u.name}</span><button className="unshare-btn" onClick={() => handleUnshare(u.id)} title="Revoke Access"><Icons.Close /></button></div>
                             ))}
                        </div>
                    </div>
                )}
                <div className="shared-section">
                    ...
                    <div className="user-list">
                        {users.filter(u => !shareItem.shared_with_ids?.includes(u.id.toString())).map(u => (
                            <div key={u.id} className="user-item" onClick={() => handleShare(u.id)}><span>{u.username}</span><button className="share-action-btn">Send</button></div>
                        ))}
