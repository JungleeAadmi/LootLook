...
function App() {
  ...
  useEffect(() => { 
      ...
      // WebSocket setup inside useEffect to prevent multiple connections
      if (token) {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setUrl('');
      // fetchItems called by socket broadcast
    } catch (err) { alert(err.message); }
    setLoading(false);
  };
  ...
  const handleRefresh = async (id) => {
    setRefreshingId(id);
    try {
        const res = await fetch(`${API_URL}/refresh/${id}`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } });
        if(res.ok) { 
            if(selectedItem?.id === id) openHistory(items.find(i => i.id === id)); 
        } else {
            const err = await res.json();
            alert("Check failed: " + (err.error || "Unknown error"));
        }
  ...
  const handleCheckAll = async () => {
      ...
      for (const item of items) {
          try { await fetch(`${API_URL}/refresh/${item.id}`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } }); } 
  ...
  const handleExport = () => {
      setMenuOpen(false);
      window.open(`${API_URL}/export?token=${token}`, '_blank');
  };
  ...
  const openHistory = async (item) => {
    ...
    try {
      const res = await fetch(`${API_URL}/history/${item.id}`, { headers: { 'Authorization': `Bearer ${token}` } });
      const json = await res.json();
  ...
  const formatDate = (dateString) => {
      ...
      return new Date(dateString).toLocaleDateString(undefined, {day:'2-digit', month:'short', year:'2-digit'});
  };
        ...
        <section className="items-grid">
            {filteredItems.map(item => (
                <article key={item.id} className={`item-card trend-${getTrend(item.current_price, item.previous_price)}`}>
                    ...
                    {/* ROW 2,3,4: CONTENT (Click -> History Graph) */}
                    ...
                    {/* ROW 5: ACTIONS (3 Columns x 2 Rows) */}
                    <div className="card-actions">
                        ...
                        <button onClick={() => openHistory(item)} className="action-btn graph" title="History"><Icons.Graph /></button>
                        
                        {/* Explicit SNIP Button */}
                        <button onClick={(e) => openImage(e, item)} className="action-btn snip-btn" title="View Screenshot"><Icons.Snip /></button>
                        
                        {/* Row B */}
                        <button onClick={() => setEditingItem(item)} className="action-btn edit" title="Edit"><Icons.Edit /></button>
                        <a href={item.url} target="_blank" rel="noreferrer" className="action-btn visit" title="Webpage"><Icons.Web /></a>
                        <button onClick={() => handleDelete(item.id)} className="action-btn remove" title="Remove"><Icons.Remove /></button>
                    </div>
                </article>
            ))}
